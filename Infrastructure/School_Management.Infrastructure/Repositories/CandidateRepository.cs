...
public class CandidateRepository : BaseRepository<Candidate>, ICandidateRepository
{
    ...
    {
        return await CreateQuery().CountAsync();
    }
    ...
    private static IQueryable<Candidate> ApplyOptionsFilter(IQueryable<Candidate> query, StudentFilterOptions options)
    {
        ...
        query = query.Where(c => c.Gender == options.Gender);
    ...
    private static bool TryParseGender(string? value, out Gender gender)
    {
        ...
        if (string.Equals(key, "áž”áŸ’ážšáž»ážŸ", StringComparison.OrdinalIgnoreCase))
        {
            gender = Gender.Male;
            return true;
        }

        if (string.Equals(key, "ážŸáŸ’ážšáž¸", StringComparison.OrdinalIgnoreCase))
        {
